...
Output {step1_output}, use ask_followup_question to confirm:
- Need to add/remove items?
- Does field framework meet requirements?

### Step 2: Web Search Supplement
Use ask_followup_question to ask for time range (e.g., last 6 months, since 2024, unlimited).
...
Spawn 1 web-researcher subtask via `new_task` (mode: `web-researcher`), **Prompt Template**:
```
## Task
...
Use ask_followup_question to ask if user has existing field definition file, if so read and merge.
...
- execution:
  - items_per_agent: Items per subtask (confirm with ask_followup_question, default: 1)
